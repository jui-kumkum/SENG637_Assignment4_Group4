...
8. Mutant: `` . This mutation is related to `Range.combine` method and appears

`return range2` and so `return null` would happen because of this mutant and will return null regardless of the scenerio but it is `KILLED` by our test case: `TestCombineWithNullAsFirstRange`
